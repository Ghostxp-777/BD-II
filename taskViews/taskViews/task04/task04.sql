...
INNER JOIN alunos ON historicosEscolares.idAluno = alunos.matricula
INNER JOIN disciplinas ON historicosEscolares.idDisciplina = disciplinas.id;
...
FROM disciplinas
INNER JOIN professores ON disciplinas.idProfessor = professores.id
...
FROM profCurso
INNER JOIN professores ON profCurso.idProfessor = professores.id
INNER JOIN disciplinas ON profCurso.idCurso = cursos.id
...
INNER JOIN alunos ON historicosEscolares.idAluno = alunos.id
INNER JOIN disciplinas ON historicosEscolares.idDisciplina = disciplinas.id
WHERE faltas <= 5;
...
SELECT
    ...
    d.nome AS nomeDisciplina
    t.id AS codigoTurma
    d.nomeProfessor AS professorResponsavel
FROM disciplinas
INNER JOIN turmas ON disciplinas.idTurma = turmas.id
WHERE qntd_alunos > 8;
...
FROM alunos AS a
LEFT JOIN historicosEscolares ON a.matricula = historicosEscolares.idAluno
...
SELECT 
    ...
    t.nome AS turma
FROM disciplinas
INNER JOIN professores ON disciplinas.idProfessorResponsavel = professores.id
INNER JOIN turmas ON disciplinas.idTurma = turmas.id
WHERE d.id NOT IN (
...
SELECT 
    ...
    h.media,
    p.nome as professor,
    (SELECT AVG(media) FROM historicosEscolares WHERE idAluno = alunos.matricula) as media_geral
FROM alunos
INNER JOIN historicosEscolares ON alunos.matricula = historicosEscolares.idAluno
INNER JOIN disciplinas ON historicosEscolares.idDisciplina = disciplinas.id
INNER JOIN professores ON disciplinas.idProfessorResponsavel = professores.id
